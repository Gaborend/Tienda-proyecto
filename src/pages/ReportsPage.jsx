    ...
    <div style={pageStyle}>
      ...
      <section style={sectionStyle}>
        ...
        <form onSubmit={handleGenerateSalesReport} style={filterSectionStyle}>
          ...
          <div style={filterItemStyle}><label style={labelStyle} htmlFor="sr_product_id">Código de Barras:</label><input style={inputStyle} type="text" id="sr_product_id" name="product_id" value={salesFilters.product_id} onChange={handleSalesFilterChange} placeholder="En venta (opcional)"/></div>
                ...
                {salesReportData.map(sale => (
                  <React.Fragment key={sale.invoice_number}>
                    <tr>
                      ...
                      <td style={tableCellStyle}>{sale.invoice_number}</td><td style={tableCellStyle}>{new Date(sale.date).toLocaleDateString()}</td>
      ...
      <section style={sectionStyle}>
        ...
        <form onSubmit={handleGenerateInventoryMovementsReport} style={filterSectionStyle}>
          ...
          <div style={filterItemStyle}><label style={labelStyle} htmlFor="im_product_id">Código de barras:</label><input style={inputStyle} type="text" id="im_product_id" name="product_id" value={inventoryMovementsFilters.product_id} onChange={handleInventoryMovementsFilterChange} placeholder="Opcional"/></div>
        ...
        {!loadingInventoryMovements && inventoryMovementsData.length > 0 && (
          <div style={{overflowX: 'auto', marginTop:'20px'}}>
            <table style={{ width: '100%', minWidth:'900px', borderCollapse: 'collapse', fontSize: '0.95em' }}>
              ...
              <tbody>{inventoryMovementsData.map((mov, index) => (<tr key={mov.id || index}> <td style={tableCellStyle}>{new Date(mov.date).toLocaleDateString()}</td><td style={tableCellStyle}>{mov.product_id}</td><td style={tableCellStyle}>{mov.product_code}</td><td style={{...tableCellStyle, color: mov.quantity_changed < 0 ? 'red' : 'green', fontWeight:'bold'}}>{mov.quantity_changed}</td><td style={tableCellStyle}>{mov.new_quantity}</td><td style={tableCellStyle}>{mov.movement_type}</td><td style={tableCellStyle}>{mov.user_id}</td><td style={tableCellStyle}>{mov.notes || '-'}</td></tr>))}</tbody>
            </table>
          </div>
        )}
      </section>
