...
function ReportsPage() {
  ...
  const fetchSalesReport = useCallback(async () => {
    ...
    try {
      ...
      if ((!data || data.length === 0) && submittedSalesFilters) {
        setSalesError("No se encontraron ventas con los filtros aplicados.");
      }
    } catch (err) {
  ...
  const handleGenerateSalesReport = (e) => {
    e.preventDefault();
    setError(''); 
    setSalesPagination(prev => ({...prev, skip:0}));
    setSubmittedSalesFilters(salesFilters); 
  };

  const handleExportSalesReport = async () => {
    if (!submittedSalesFilters || salesReportData.length === 0) {
        alert("Por favor, genere un reporte con datos primero antes de exportar.");
        return;
    }
    setLoadingSalesExport(true);
    setSalesError(''); 
    ...
    try {
        ...
        setSalesError("Error al exportar reporte: " + (err.detail || err.message || "Error desconocido"));
    } finally {
        setLoadingSalesExport(false);
    }
  };
  
  const toggleSaleDetails = (saleInvoiceNumber) => {
    setExpandedSaleId(expandedSaleId === saleInvoiceNumber ? null : saleInvoiceNumber);
  };

  const handleSalesNextPage = () => {
    if (salesReportData.length < salesPagination.limit && salesPagination.skip > 0) { return; }
    if (salesReportData.length === 0 && salesPagination.skip === 0 && submittedSalesFilters) { return; }
    setSalesPagination(prev => ({ ...prev, skip: prev.skip + prev.limit }));
  };

  const handleSalesPreviousPage = () => {
    setSalesPagination(prev => ({ ...prev, skip: Math.max(0, prev.skip - prev.limit) }));
  };
  ...
  const fetchInventoryMovementsReport = useCallback(async () => {
    if (!submittedInventoryMovementsFilters) {
      setInventoryMovementsData([]); return;
    }
    setLoadingInventoryMovements(true);
    setInventoryMovementsError('');
    const params = { 
      ...submittedInventoryMovementsFilters,
      skip: inventoryMovementsPagination.skip,
      limit: inventoryMovementsPagination.limit
    };
    Object.keys(params).forEach(key => {
      if (params[key] === '' || params[key] === null) delete params[key];
    });
    if (params.product_id && params.product_id.trim() !== '') params.product_id = parseInt(params.product_id, 10); else delete params.product_id;
    if (params.user_id && params.user_id.trim() !== '') params.user_id = parseInt(params.user_id, 10); else delete params.user_id;
    
    try {
      const data = await reportsService.getInventoryMovementsReport(params);
      setInventoryMovementsData(data || []);
      if ((!data || data.length === 0) && submittedInventoryMovementsFilters) {
        setInventoryMovementsError("No se encontraron movimientos de inventario con los filtros aplicados.");
      }
    } catch (err) {
      console.error("Error al generar reporte de movimientos de inventario:", err);
      setInventoryMovementsError("Error al generar reporte: " + (err.detail || err.message));
      setInventoryMovementsData([]);
    } finally {
      setLoadingInventoryMovements(false);
    }
  ...
  const handleGenerateInventoryMovementsReport = (e) => {
    e.preventDefault();
    setError(''); 
    setInventoryMovementsPagination(prev => ({...prev, skip:0}));
    setSubmittedInventoryMovementsFilters(inventoryMovementsFilters);
  };

  const handleExportInventoryMovementsReport = async () => {
    if (!submittedInventoryMovementsFilters || inventoryMovementsData.length === 0) {
        alert("Por favor, genere un reporte con datos primero antes de exportar."); return;
    }
    setLoadingInventoryMovementsExport(true);
    setInventoryMovementsError(''); 
    ...
    try {
        await reportsService.downloadInventoryMovementsReport(params);
    } catch (err) {
        console.error("Error al exportar reporte de movimientos:", err);
        setInventoryMovementsError("Error al exportar reporte: " + (err.detail || err.message || "Error desconocido"));
    } finally {
        setLoadingInventoryMovementsExport(false);
    }
  };

  const handleInventoryMovementsNextPage = () => {
    if (inventoryMovementsData.length < inventoryMovementsPagination.limit) return;
    setInventoryMovementsPagination(prev => ({ ...prev, skip: prev.skip + prev.limit }));
  };

  const handleInventoryMovementsPreviousPage = () => {
    setInventoryMovementsPagination(prev => ({ ...prev, skip: Math.max(0, prev.skip - prev.limit) }));
  };
  ...
  const handleServicesPerformedFilterChange = (e) => {
    ...
    setServicesPerformedFilters(prev => ({ 
        ...prev, 
  ...
  const handleGenerateServicesPerformedReport = (e) => {
    e.preventDefault();
    setError('');
    setServicesPerformedPagination(prev => ({...prev, skip:0}));
    setSubmittedServicesPerformedFilters(servicesPerformedFilters);
  };
  ...
  const handleExportServicesPerformedReport = async () => {
    ...
    try {
        ...
        setLoadingServicesPerformedExport(false);
    }
  };

  const handleServicesPerformedNextPage = () => {
    if (servicesPerformedData.length < servicesPerformedPagination.limit) return;
    setServicesPerformedPagination(prev => ({ ...prev, skip: prev.skip + prev.limit }));
  };

  const handleServicesPerformedPreviousPage = () => {
    setServicesPerformedPagination(prev => ({ ...prev, skip: Math.max(0, prev.skip - prev.limit) }));
  };
    ...
    <div style={pageStyle}>
                  ...
                  <React.Fragment key={sale.invoice_number}>
                    <tr>
                      <td style={tableCellStyle}>
                        <button onClick={() => toggleSaleDetails(sale.invoice_number)} style={{padding:'3px 6px', fontSize:'0.8em', cursor:'pointer', border:'1px solid #ccc', background:'#f0f0f0'}}>
                          {expandedSaleId === sale.invoice_number ? '−' : '+'}
                        </button>
                      </td>
                      <td style={tableCellStyle}>{sale.invoice_number}</td>
                      <td style={tableCellStyle}>{new Date(sale.date).toLocaleString()}</td>
                      <td style={tableCellStyle}>{sale.customer_name}</td>
                      <td style={tableCellStyle}>{sale.customer_document || 'N/A'}</td>
                      <td style={{...tableCellStyle, textAlign:'right'}}>{sale.total_amount.toFixed(2)}</td>
                      <td style={tableCellStyle}>{sale.payment_method}</td>
                      <td style={tableCellStyle}>
                        <span style={{ color: sale.status === 'cancelled' ? 'red' : 'green', fontWeight:'bold' }}>
                    ...
                    {expandedSaleId === sale.invoice_number && (
                      <tr>
                        <td colSpan="8" style={{padding: '10px 15px 10px 40px', backgroundColor:'#fdfdfd', borderBottom:'1px solid #ddd'}}>
                          <h4 style={{marginTop:'5px', marginBottom:'8px', fontSize:'0.95em', color:'#444'}}>Items de la Factura:</h4>
                          <table style={itemsSubTableStyle}>
                            <thead>
                              <tr>
                                <th style={itemsSubThStyle}>Descripción</th>
                                <th style={{...itemsSubThStyle, textAlign:'right'}}>Cant.</th>
                                <th style={{...itemsSubThStyle, textAlign:'right'}}>P. Unit.</th>
                                <th style={{...itemsSubThStyle, textAlign:'right'}}>Total Ítem</th>
                              </tr>
                            </thead>
                            <tbody>
                              {(sale.items || []).map((item, index) => (
                                <tr key={index}>
                                  <td style={itemsSubTdStyle}>{item.description}</td>
                                  ...
                                  <td style={{...itemsSubTdStyle, textAlign:'right'}}>{item.total_item_price.toFixed(2)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
      ...
      {submittedSalesFilters && ((!loadingSales && salesReportData.length > 0) || salesPagination.skip > 0) ? (
          <div style={paginationContainerStyle}>
            <button onClick={handleSalesPreviousPage} disabled={salesPagination.skip === 0 || loadingSales} style={{...buttonStyle, backgroundColor: '#6c757d', color: 'white'}}>
                Anterior
            </button>
            <span>Página {Math.floor(salesPagination.skip / salesPagination.limit) + 1}</span>
            <button onClick={handleSalesNextPage} disabled={salesReportData.length < salesPagination.limit || loadingSales} style={{...buttonStyle, backgroundColor: '#6c757d', color: 'white'}}>
                Siguiente
            </button>
          </div>
      ) : null}
      ...
      <section style={sectionStyle}>
        ...
        <form onSubmit={handleGenerateInventoryMovementsReport} style={filterSectionStyle}>
          ...
          <div style={filterItemStyle}><label style={labelStyle} htmlFor="im_movement_type">Tipo Movimiento:</label><input style={inputStyle} type="text" id="im_movement_type" name="movement_type" value={inventoryMovementsFilters.movement_type} onChange={handleInventoryMovementsFilterChange} placeholder="Ej: venta, compra (opcional)"/></div>
          ...
          <div style={{display:'flex', gap:'10px', width:'100%', marginTop:'10px'}}>
            <button type="submit" style={{ ...buttonStyle, backgroundColor: '#28a745', flexGrow:1}} disabled={loadingInventoryMovements}>
              {loadingInventoryMovements ? 'Generando...' : 'Generar Reporte Mov. Inventario'}
            </button>
            <button type="button" onClick={handleExportInventoryMovementsReport} style={{ ...buttonStyle, backgroundColor: '#17a2b8', flexGrow:1}} disabled={loadingInventoryMovementsExport || inventoryMovementsData.length === 0}>
              {loadingInventoryMovementsExport ? 'Exportando...' : 'Exportar a Excel'}
            </button>
          </div>
        </form>
        ...
        {!loadingInventoryMovements && submittedInventoryMovementsFilters && inventoryMovementsData.length === 0 && !inventoryMovementsError && (
             <p style={{textAlign:'center', padding:'20px', fontStyle:'italic'}}>No se encontraron movimientos de inventario con los filtros aplicados.</p>
        )}
        {!loadingInventoryMovements && inventoryMovementsData.length > 0 && (
          <div style={{overflowX: 'auto', marginTop:'20px'}}>
            <table style={{ width: '100%', minWidth:'900px', borderCollapse: 'collapse', fontSize: '0.95em' }}>
              <thead>
                <tr>
                  <th style={tableHeaderStyle}>Fecha</th><th style={tableHeaderStyle}>ID Prod.</th>
                  <th style={tableHeaderStyle}>Cód. Prod.</th><th style={tableHeaderStyle}>Cant. Modif.</th>
                  <th style={tableHeaderStyle}>Nueva Cant.</th><th style={tableHeaderStyle}>Tipo Mov.</th>
                  <th style={tableHeaderStyle}>Usuario (ID)</th><th style={tableHeaderStyle}>Notas</th>
                </tr>
              </thead>
              <tbody>
                {inventoryMovementsData.map((mov, index) => (
                  <tr key={mov.id || index}> 
                    <td style={tableCellStyle}>{new Date(mov.date).toLocaleString()}</td>
                    <td style={tableCellStyle}>{mov.product_id}</td>
                    <td style={tableCellStyle}>{mov.product_code}</td>
                    <td style={{...tableCellStyle, color: mov.quantity_changed < 0 ? 'red' : 'green', fontWeight:'bold'}}>{mov.quantity_changed}</td>
                    <td style={tableCellStyle}>{mov.new_quantity}</td>
                    <td style={tableCellStyle}>{mov.movement_type}</td>
                    <td style={tableCellStyle}>{mov.user_id}</td>
                    <td style={tableCellStyle}>{mov.notes || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {submittedInventoryMovementsFilters && ((!loadingInventoryMovements && inventoryMovementsData.length > 0) || inventoryMovementsPagination.skip > 0) ? (
          <div style={paginationContainerStyle}>
            <button onClick={handleInventoryMovementsPreviousPage} disabled={inventoryMovementsPagination.skip === 0 || loadingInventoryMovements} style={{...buttonStyle, backgroundColor: '#6c757d', color: 'white'}}>
                Anterior
            </button>
            <span>Página {Math.floor(inventoryMovementsPagination.skip / inventoryMovementsPagination.limit) + 1}</span>
            <button onClick={handleInventoryMovementsNextPage} disabled={inventoryMovementsData.length < inventoryMovementsPagination.limit || loadingInventoryMovements} style={{...buttonStyle, backgroundColor: '#6c757d', color: 'white'}}>
                Siguiente
            </button>
          </div>
      ) : null}
      ...
      <section style={sectionStyle}>
         ...
         <div style={filterItemStyle}><label style={labelStyle} htmlFor="sp_start_date">Desde:</label><input style={inputStyle} type="date" id="sp_start_date" name="start_date" value={servicesPerformedFilters.start_date} onChange={handleServicesPerformedFilterChange} /></div>
          ...
          <div style={{display:'flex', gap:'10px', width:'100%', marginTop:'10px'}}>
            <button type="submit" style={{ ...buttonStyle, backgroundColor: '#28a745', flexGrow:1}} disabled={loadingServicesPerformed}>
              {loadingServicesPerformed ? 'Generando...' : 'Generar Reporte Servicios'}
            </button>
            <button type="button" onClick={handleExportServicesPerformedReport} style={{ ...buttonStyle, backgroundColor: '#17a2b8', flexGrow:1}} disabled={loadingServicesPerformedExport || servicesPerformedData.length === 0}>
              {loadingServicesPerformedExport ? 'Exportando...' : 'Exportar a Excel'}
            </button>
          </div>
        </form>
        ...
        {!loadingServicesPerformed && submittedServicesPerformedFilters && servicesPerformedData.length === 0 && !servicesPerformedError && (
             <p style={{textAlign:'center', padding:'20px', fontStyle:'italic'}}>No se encontraron servicios realizados con los filtros aplicados.</p>
        )}
        {!loadingServicesPerformed && servicesPerformedData.length > 0 && (
          <div style={{overflowX: 'auto', marginTop:'20px'}}>
            <table style={{ width: '100%', minWidth:'900px', borderCollapse: 'collapse', fontSize: '0.95em' }}>
              <thead>
                <tr>
                  <th style={tableHeaderStyle}>Fecha Venta</th><th style={tableHeaderStyle}>Factura N°</th>
                  <th style={tableHeaderStyle}>Servicio (ID)</th><th style={tableHeaderStyle}>Cód. Servicio</th>
                  <th style={tableHeaderStyle}>Descripción Servicio</th><th style={{...tableHeaderStyle, textAlign:'right'}}>Cant.</th>
                  <th style={{...tableHeaderStyle, textAlign:'right'}}>P. Unit.</th><th style={{...tableHeaderStyle, textAlign:'right'}}>P. Total Serv.</th>
                  <th style={tableHeaderStyle}>Cliente (ID)</th><th style={tableHeaderStyle}>Nombre Cliente</th>
                </tr>
              </thead>
              <tbody>
                {servicesPerformedData.map((serv, index) => (
                  <tr key={`${serv.invoice_number}-${serv.service_id}-${index}`}>
                    <td style={tableCellStyle}>{new Date(serv.sale_date).toLocaleString()}</td>
                    <td style={tableCellStyle}>{serv.invoice_number}</td>
                    <td style={tableCellStyle}>{serv.service_id === 0 ? 'Temporal' : serv.service_id}</td>
                    <td style={tableCellStyle}>{serv.service_code || '-'}</td>
                    <td style={tableCellStyle}>{serv.service_description}</td>
                    <td style={{...tableCellStyle, textAlign:'right'}}>{serv.quantity}</td>
                    <td style={{...tableCellStyle, textAlign:'right'}}>{serv.unit_price.toFixed(2)}</td>
                    <td style={{...tableCellStyle, textAlign:'right'}}>{serv.total_service_price.toFixed(2)}</td>
                    <td style={tableCellStyle}>{serv.customer_id}</td>
                    <td style={tableCellStyle}>{serv.customer_name}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {submittedServicesPerformedFilters && ((!loadingServicesPerformed && servicesPerformedData.length > 0) || servicesPerformedPagination.skip > 0) ? (
          <div style={paginationContainerStyle}>
            <button onClick={handleServicesPerformedPreviousPage} disabled={servicesPerformedPagination.skip === 0 || loadingServicesPerformed} style={{...buttonStyle, backgroundColor: '#6c757d', color: 'white'}}>
                Anterior
            </button>
            <span>Página {Math.floor(servicesPerformedPagination.skip / servicesPerformedPagination.limit) + 1}</span>
            <button onClick={handleServicesPerformedNextPage} disabled={servicesPerformedData.length < servicesPerformedPagination.limit || loadingServicesPerformed} style={{...buttonStyle, backgroundColor: '#6c757d', color: 'white'}}>
                Siguiente
            </button>
          </div>
      ) : null}
